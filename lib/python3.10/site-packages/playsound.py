...
def _playsoundWin(sound:str, kind:str='system', block:bool=False):
    '''
    ...
    '''
    if not s.allSound or not s.sound[kind] or not sound: return

    sound = f"{s.TFP}Assets{s.s}sounds{s.s}{sound}.wav"
    ...
    if any((c in sound for c in ' "\'()')):
        ...
        try:
            _playsoundWin(tempPath, block=block)
        finally:
            remove(tempPath)
        return
...
def _playsoundOSX(sound:str, kind:str='system', block:bool=False):
    '''
    ...
    ## Rudventure:
        Please note that this code has been partially modified and deleted for convenient and free use of this game.
    '''
    # try:
    ...
    url = NSURL.URLWithString_(_handlePathOSX(f"{s.TFP}Assets{s.s}sounds{s.s}{sound}.wav"))
...
def _playsoundNix(sound:str, kind:str='system', block:bool=False):

...
def _system(stdscr,
    ...
    while 1:
        ...
        currArrowColor = menuKeys[currSelectRow][currSelectColumn][0] or arrowColorInit

        arrow[currSelectRow][currSelectColumn] = f'{currArrowColor}{icon}'

        # 색 전염 방지
        if currSelectRow < (len(menuKeys)-1):
            arrow[currSelectRow+1][currSelectColumn] = f"{cc['end']} "

        if currSelectRow>0 and currSelectColumn<(maxLine-1):
            arrow[0][currSelectColumn+1]  = f"{cc['end']} "

        if (currSelectColumn+1) < maxLine:
            arrow[currSelectRow][currSelectColumn+1] = f"{cc['end']} "

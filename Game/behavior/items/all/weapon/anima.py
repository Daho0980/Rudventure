...
class Anima(ItemBehavior):
    def collect(self) -> None:
        if s.playerIdentity == 'upload':
            target = c.specialComment['anima']['get']
            sayCmt(target['cmt'], target['prob'])

    def use(self) -> None:
        if (targetIndex:=inventory.search('animus')) == -1:
            if s.playerIdentity == 'upload':
                target = c.specialComment['anima']['merge']['failure']
                sayCmt(target['cmt'], target['prob'])
            ...
            if s.playerIdentity == 'upload':
                target = c.specialComment['anima']['merge']['success']
                sayCmt(target['cmt'], target['prob'])

            else: addLog("아니마가 융합되었습니다!")
    ...
    def put(self) -> None:
        if s.playerIdentity == 'upload':
            target = c.specialComment['anima']['put']
            sayCmt(target['cmt'], target['prob'])
    ...
    def tread(self, itemId:str, itemType:str, payload:int) -> None:
        if s.playerIdentity == 'upload':
            target = c.specialComment['anima']['tread']
            sayCmt(target['cmt'], target['prob'])

behaviorMap['weapon']["anima"] = Anima()
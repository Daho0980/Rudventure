...
def gridMapReturn(grid:list, blank=0, center=False):
    ...
    if center == True   :
        ...
        toolY, toolX = 4-status.Dy, 4-status.Dx
        ...
        for row in range(len(DisplayMap)):
            for column in range(len(DisplayMap[row])):
                ...
                if FixY >= 0 and FixY <= len(DisplayMap)-1 and FixX >= 0 and FixX <= len(DisplayMap[row])-1 and len(grid[row][column]) > 0:
                    if row == status.Dy and column == status.Dx: DisplayMap[FixY][FixX] = f"\033[45m{grid[status.Dy][status.Dx]['roomIcon']}{status.colors['end']}"
                    else:
                        match grid[row][column]["isPlayerVisited"]:
                            case 0: DisplayMap[FixY][FixX] = ' '
                            case 1: DisplayMap[FixY][FixX] = f"{status.colors['lP']}?{status.colors['end']}"
                            case 2: DisplayMap[FixY][FixX] = grid[row][column]["roomIcon"]

    elif center == False:
        DisplayMap = GraphicMaker(grid)

        DisplayMap[status.Dy][status.Dx] = f"\033[45m{DisplayMap[status.Dy][status.Dx]}{status.colors['end']}"
        for row in range(len(DisplayMap)):
            for column in range(len(DisplayMap[row])):
                if len(grid[row][column]) > 0:
                    match grid[row][column]["isPlayerVisited"]:
                        case 0: DisplayMap[row][column] = ' '
                        case 1: DisplayMap[row][column] = f"{status.colors['lP']}?{status.colors['end']}"
        ...
        for column in range(len(output[row])):
            if len(output[row][column]) > 0:
                ...
                for DIE in range(len(RDP)):
                    if RDP[DIE] == 1:
                        baseMap[GRDP[DIE][0]][GRDP[DIE][1]] = status.R
                        if DIE in [0, 2]  : baseMap[GRDP[DIE][0]][GRDP[DIE][1]-1], baseMap[GRDP[DIE][0]][GRDP[DIE][1]+1] = status.R, status.R
                        elif DIE in [1, 3]: baseMap[GRDP[DIE][0]-1][GRDP[DIE][1]], baseMap[GRDP[DIE][0]+1][GRDP[DIE][1]] = status.R, status.R
            ...
            if len(output[row][column]) > 0:
                ...
                for i in range(len(p)):
                    if len(output[p[i][0]][p[i][1]]) > 0 and random.randrange(0, 3) == 1:
                        ...
                        output[row][column]['room'][grd[i][0][0]][grd[i][0][1]]      = status.R
                        if dp[i][0] in ['U', 'D']  : output[row][column]['room'][grd[i][0][0]][grd[i][0][1]-1], output[row][column]['room'][grd[i][0][0]][grd[i][0][1]+1] = status.R, status.R
                        elif dp[i][0] in ['L', 'R']: output[row][column]['room'][grd[i][0][0]-1][grd[i][0][1]], output[row][column]['room'][grd[i][0][0]+1][grd[i][0][1]] = status.R, status.R
                        output[p[i][0]][p[i][1]]['room'][grd[i][1][0]][grd[i][1][1]] = status.R
                        if dp[i][0] in ['U', 'D']  : output[p[i][0]][p[i][1]]['room'][grd[i][1][0]][grd[i][1][1]-1], output[p[i][0]][p[i][1]]['room'][grd[i][1][0]][grd[i][1][1]+1] = status.R, status.R
                        elif dp[i][0] in ['L', 'R']: output[p[i][0]][p[i][1]]['room'][grd[i][1][0]-1][grd[i][1][1]], output[p[i][0]][p[i][1]]['room'][grd[i][1][0]+1][grd[i][1][1]] = status.R, status.R
    return output
...
def initBranch(Map:list, y:int, x:int, rawPrint=False):
    """
...
def DungeonMaker():
    ...
    output                       = makeRoom(deleteBlankData(initBranch(output, 4, 4, rawPrint=True)))
    print("생성 완료!")
    time.sleep(0.1)
